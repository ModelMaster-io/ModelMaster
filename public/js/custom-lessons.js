			...
			success: function (response) {
				...
				if (response.status == 2) {
					...
					if (rgt_cells.length > 0) {
						...
						for (var i = 0; i < rgt_cells.length; i++) {
							keys = Object.keys(rgt_cells[i]);
							row = keys[0];
							col = rgt_cells[i][row];
							sheet.comments.remove(parseInt(row), col);
						}
					...
					if (response.status == 0) {
						...
						// for (var i = 0; i < wrong_cells.length; i++) {
						// 	keys = Object.keys(wrong_cells[i]);
						// 	row = keys[0];
						// 	col = wrong_cells[i][row];
						// 	var comment = sheet.comments.add(parseInt(row), col, response.error_msg);
				...
				if (historical_obj.status == 1) {
					...
					if (historical_obj.hData != '') {
						...
						jQuery.each(automaic_data, function (row, value) {
							...
							jQuery.each(value, function (col) {
